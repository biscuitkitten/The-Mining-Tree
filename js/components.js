...
// data = a function returning the content (actually HTML)
				...
				if (!this.interval && layers[this.layer].clickables[this.data].onHold) {
					this.interval = setInterval((function() {
						...
						if(this.time >= 0 && run(c.canClick, c)) {
							run(c.onHold, c)
							// Fix added to prevent clickable from getting stuck on permanently
							if(player.tab == "lavaAlert") this.stop()
						}	
						this.time = this.time+1
					}).bind(this), 50)}
			},
		...
		methods: {
			start() {
				if (!this.interval && layers[this.layer].grid.onHold) {
					this.interval = setInterval((function() {
						if(this.time >= 0 && gridRun(this.layer, 'getCanClick', player[this.layer].grid[this.data], this.data)) {
	...
	Vue.component('overlay-inv', systemComponents['overlay-inv'])
	Vue.component('overlay-money', systemComponents['overlay-money'])
