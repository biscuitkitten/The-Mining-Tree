...
function updateTemp() {
	...
	tmp.displayInv = []
	for (thing in displayInv){
		let text = displayInv[thing]
		if (isFunction(text)) text = text()
		tmp.displayInv.push(text) 
	}
	tmp.displayMoney = []
	for (thing in displayMoney){
		let text = displayMoney[thing]
		if (isFunction(text)) text = text()
		tmp.displayMoney.push(text) 
	}
