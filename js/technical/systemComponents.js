var systemComponents = {
    ...
    'info-tab': {
		...
		<br>
		Epic Mining 2 made by QuintinityZero
		<br><br>
    ...
    'options-tab': {
        ...
        <table>
            ...
            <!--</tr>-->
            <tr>
                <td><button class="opt" onclick="toggleOpt('headlampOn')">Enable Headlamp:<br>{{ options.headlampOn?"ON":"OFF" }}</button></td>
                <td><button class="opt" onclick="toggleOpt('showInv')">Expand Side Inventory:<br>{{ options.showInv?"ON":"OFF" }}</button></td>
                <td><button class="opt" onclick="toggleOpt('showManualHole')">Show Tunnel Down Button:<br>{{ options.showManualHole?"ON":"OFF" }}</button></td>
	...
	'overlay-money': {
		template: `			
		<div class="overlayMoney" style="margin-top:7px; margin-right:2px; padding-bottom: 7px; padding-top: 7px; z-index: 1000; right: 0; top: 0; width: 10%; position: fixed; font-size: 14px">
			<div v-for="thing in tmp.displayMoney" class="overlayThing"><span v-if="thing" v-html="thing"></span></div>
		</div>
	`
    },
	...
	'overlay-inv': {
		template: `			
		<div class="overlayInv" style="margin-bottom:7px; margin-right:7px; padding-bottom: 7px; padding-top: 7px; z-index: 1000; right: 0; bottom: 0; width: 10%; background: rgba(0,0,0,0.5); border: 2px solid black; position: fixed; line-height:1.1; font-size: 16px">
			<div v-for="thing in tmp.displayInv" class="overlayThing"><span v-if="thing" v-html="thing"></span></div>
		</div>
	`
    },	
