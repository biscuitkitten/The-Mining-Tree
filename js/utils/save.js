...
function save(force) {
	...
	localStorage.setItem(getModID(), encodeSave(unescape(encodeURIComponent(JSON.stringify(player)))));
	localStorage.setItem(getModID()+"_options", encodeSave(unescape(encodeURIComponent(JSON.stringify(options)))));
...
function load() {
	...
	if (get === null || get === undefined) {
		...
		player = Object.assign(getStartPlayer(), JSON.parse(decodeURIComponent(escape(decodeSave(get)))));
		fixSave();
		loadOptions();
	}
...
function loadOptions() {
	...
	if (get2) 
		options = Object.assign(getStartOptions(), JSON.parse(decodeURIComponent(escape(decodeSave(get2)))));
	else 
		options = getStartOptions()
...
function exportSave() {
	//if (NaNalert) return
	let str = encodeSave(JSON.stringify(player));
...
function importSave(imported = undefined, forced = false) {
	...
	try {
		tempPlr = Object.assign(getStartPlayer(), JSON.parse(decodeSave(imported)));
...
// Due to massive save file size, a better compression algorithm is used
